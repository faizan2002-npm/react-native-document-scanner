...
- (void) didDetectRectangle:(CIRectangleFeature *)rectangle withType:(IPDFRectangeType)type {
    ...
    // For Fabric, events are dispatched through the view manager
    // The codegen will generate the proper event dispatching mechanism
    // For now, we'll use the old event block system as a fallback
...
- (void) capture {
    [self captureImageWithCompletionHander:^(UIImage *croppedImage, UIImage *initialImage, CIRectangleFeature *rectangleFeature) {
        ...
        /*
         ...
         Sorry about that.
         Flattened for codegen compatibility.
         */
        float topLeftX, topLeftY, topRightX, topRightY, bottomLeftX, bottomLeftY, bottomRightX, bottomRightY;
        if (rectangleFeature) {
            // Map from landscape to portrait coordinates
            topLeftX = rectangleFeature.bottomLeft.y;
            topLeftY = rectangleFeature.bottomLeft.x + 30;
            topRightX = rectangleFeature.topLeft.y;
            topRightY = rectangleFeature.topLeft.x + 30;
            bottomLeftX = rectangleFeature.bottomRight.y;
            bottomLeftY = rectangleFeature.bottomRight.x;
            bottomRightX = rectangleFeature.topRight.y;
            bottomRightY = rectangleFeature.topRight.x;
        } else {
            // Default to zeros when no rectangle detected
            topLeftX = topLeftY = topRightX = topRightY = bottomLeftX = bottomLeftY = bottomRightX = bottomRightY = 0.0;
        }
